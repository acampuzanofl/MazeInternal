...
enum THREADINFOCLASS
{
    ...
    ThreadQuerySetWin32StartAddress = 9, //not reall sure why?
};
...
//copy teb of target thread to hack thread
void copyTebToCurrentDLL( void* TebBaseAddress)
{
...
DWORD WINAPI HackThread(HMODULE hModule)
{
    ...
    while (true)
    {
        ...
        if (GetAsyncKeyState(VK_NUMPAD1) & 1) { copyTebToCurrentDLL(findTeb()); }
        ...
        if (GetAsyncKeyState(VK_NUMPAD3) & 1) 
        {
            ...
            {
                gotCheckpoint((void*)raceManagerObject, 0, 1000);
            }
